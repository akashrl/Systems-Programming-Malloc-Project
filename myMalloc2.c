...
static inline header * allocate_object(size_t raw_size) {
  ...
  size_t c;
	...
	else if(block_size <=(N_LISTS-1)*8 + ALLOC_HEADER_SIZE)
	{
...
static inline void deallocate_object(void * p) {
  // TODO implement deallocation
	...
	else if (get_state(rightHeader) == UNALLOCATED && get_state(leftHeader) == UNALLOCATED)
	{
		...
		if (get_size(leftHeader) >ALLOC_HEADER_SIZE  + (N_LISTS-1)*8)
			flag = 1;
	...
	else if (get_state(rightHeader) == UNALLOCATED)
	{
		...
		if (get_size(rightHeader) > ALLOC_HEADER_SIZE + (N_LISTS-1)*8)
                        flag = 1;
...
static inline void fix_free_lists(header * current, size_t size, int flag, header * old)
{
	if(flag == 0)
	{
		...
		if(size<=ALLOC_HEADER_SIZE + 8*(N_LISTS-1)) {
