...
static inline header * allocate_object(size_t raw_size) {
  ...
  size_t c;
	...
	if (block_size< sizeof(header)) block_size = sizeof(header);

	if(raw_size == 0) // If size of allocable block does not exist, return NULL
	{
		return NULL;
	}
	else if(block_size <= (N_LISTS-1)*longSize + ALLOC_HEADER_SIZE)
	{
		int x = (block_size-ALLOC_HEADER_SIZE)/longSize - 1;
...
static inline header * split_block(header *temp,size_t block_size)
{
	...
	int i = (get_size(temp) -ALLOC_HEADER_SIZE)/longSize - 1;
